...
const GSAPscrollTrigger = () => {
  ...
  useGSAP(
    () => {
      ...
      gsap.set(".animate-text-5", { autoAlpha: 0, scale: 0.5 });
      ...
      // Animate fourth line
      t1.from(".animate-text-4 h2", {
        yPercent: 130,
        ease: "power1.inOut",
        duration: 1,
      });

      // Animate text-5 with a scale effect
      t1.to(".animate-text-5", {
        scale: 1,
        autoAlpha: 1,
        ease: "elastic.out(2, 1)", // Elastic effect withy parameters (how much far to bounce, how fast(lower is faster and higher is slower))
        duration: 1,
      });

      // Animate the underline
      t1.from(".animate-line", {
       scaleX: 0, // Animate the scale on the X-axis from 0 to 1
  transformOrigin: "left center", // Ensure it scales from the left side
        ease: "power1.inOut",
        duration: 1,
      });

      //Animate section
      t1.to(".animate-section", {
        scale: 100,
        ease: "power1.inOut",
        duration: 8,
      });
  ...
  return (
    <div ref={container} className="overflow-hidden relative min-h-screen w-full flex items-center justify-center py-20 px-4 z-3 bg-gray-100 opacity-80 ">
    <section className="animate-section">
      {/* Faint background shape */}
      <div className="absolute inset-0 flex items-center justify-center -z-10">
        <div className="w-1/2 h-48 sm:h-64 opacity-10" />
      </div>
      ...
      <div className="flex flex-col items-center text-center space-y-2">
        ...
        {/* Line 4 */}
        <div className="flex items-center justify-center flex-wrap gap-4 sm:gap-8">
          <div className="animate-text-4 overflow-hidden">
          ...
          <BracketBorderImage />
          <Link href="/creators">
            <div className="text-5xl sm:text-7xl md:text-8xl font-extrabold text-gray-900 tracking-tighter">
              <h2 className="animate-text-5">CREATORS ↗</h2>{" "}
              {/*animate-text-5 class for the fifth line*/}
              {/* Underline */}
              <div className="w-full pt-4">
                <div className="h-1 bg-black animate-line"></div>
              </div>
            </div>
          </Link>
        </div>
